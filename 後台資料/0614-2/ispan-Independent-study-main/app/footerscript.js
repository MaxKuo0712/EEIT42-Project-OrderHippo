            ...
            $('.orderCBtn').click(function () {
                ...
                $.ajax({
                    ...
                    success: (res, status) => {
                        ...
                        // 信用卡刷退流程
                        $.ajax({
                            url: `http://localhost:${localhost}/api/${storeId}/payments?token=${newToken}&orderid=${putress.ORDER_ID}`,
                            method: 'GET',
                            success: (res, status) => {
                                if (res[0].PAYMENT_CATEGORY === '1') {
                                    $.ajax({
                                        url: `http://localhost:${localhost}/api/${storeId}/striperefund?token=${newToken}&paymentintent=${res[0].PAYMENT_ID}`,
                                        method: 'POST',
                                        success: (res, status) => {
                                            console.log("退款成功");
                                        }, error: err => {
